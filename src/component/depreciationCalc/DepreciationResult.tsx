...
import {useDepreciation} from "../../hook/DepreciationHook";
...
const DepreciationResult: React.FunctionComponent<DepreciationResultProps> = (props) => {
    const depreciationHook = useDepreciation();
    const depreciationGroup = useContext<DepreciationGroups>(DepreciationCalcContext).depreciationGroups[props.depreciationGroup - 1];
    ...
    useEffect(() => {
        const depreciation = depreciationHook.calculateDepreciation(currentValues.yearOrder, currentValues.balance, false, depreciationGroup);
            ...
            <Table celled>
                ...
                <Table.Body>
                    {resultRows.map((row, index) => {
                        return <Table.Row key={`result-row-${index}`}>
                            <Table.Cell>
